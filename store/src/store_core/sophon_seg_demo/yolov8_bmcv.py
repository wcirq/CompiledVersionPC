        ...
        while not end_flag:
            ...
            if (len(frame_list) == batch_size or end_flag) and len(frame_list):
                ...
                for i, frame in enumerate(frame_list):
                    ...
                    save_name = os.path.join(output_img_dir, save_basename.replace('.jpg', ''))
                    drawed_mat = yolov8.postprocess.draw_and_visualize(save_name,frame_list[i].asmat(), boxes, segments, vis=False, save=False)
                    drawed_bmimg = yolov8.bmcv.mat_to_bm_image(drawed_mat)
